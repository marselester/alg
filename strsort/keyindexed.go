...
The first step is to count the frequency of occurrence of each key.
...
Key-indexed counting uses 11*n + 4*radix + 1 array accesses to stably sort n items whose keys are integers
between 0 and radix-1.
*/
...
func keyIndexedCounting(students []student, radix int) {
	// Count the frequency of occurrence of each key.
	count := make([]int, radix+1)
